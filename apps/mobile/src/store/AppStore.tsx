...
const AppStoreContext = createContext<StoreContextValue | undefined>(undefined);
...
export function AppStoreProvider({ children }: PropsWithChildren) {
    ...
    async function hydrate() {
      try {
        ...
        if (mounted) {
          ...
          setError("Saved data could not be recovered, so the demo projects were loaded instead.");
        }
      } finally {
        if (mounted) {
  ...
  useEffect(() => {
    ...
    saveAppData(data).catch((saveError) => {
      console.warn("Unable to save app data", saveError);
      setError("Changes were made, but saving locally failed.");
    });
  }, [data, initializing]);
  ...
  function rememberWeatherCache(
    cacheEntry: WeatherCacheEntry,
    projectId?: string,
  ) {
    setData((current) => ({
      ...current,
        ...
        const baseRanges =
          input.ranges && input.ranges.length > 0
            ...
            : existingRanges.length > 0
              ? fillBlankRangeLabels(existingRanges.map((range, index) => ({
                  ...range,
                  projectId,
                  sortOrder: index,
                })))
    ...
    async resetProjectProgress(projectId) {
      return withBusy(async () => {
        setData((current) => ({
          ...current,
          progressRows: current.progressRows.map((row) =>
            row.projectId === projectId ? { ...row, completed: false, completedAt: null } : row,
          ),
        }));
      });
    },
        ...
        setData((current) => ({
          ...
          projects: current.projects.map((project) =>
            project.id === projectId
              ? { ...project, archivedAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
              : project,
          ),
        }));
      });
    ...
    async deleteProject(projectId) {
      return withBusy(async () => {
        setData((current) => ({
          ...
          temperatureDays: current.temperatureDays.filter((day) => day.projectId !== projectId),
